...
namespace Neo.SymVM
{
    ...
    public class SymInstruction
    {
        /// <summary>
        /// Represents the instruction with <see cref="OpCode.RET"/>.
        /// </summary>
        public static SymInstruction RET { get; } = new SymInstruction(OpCode.RET);
        ...
        public SymInstruction(OpCode opcode)
        {
            OpCode = opcode;
            if (!Enum.IsDefined(typeof(OpCode), opcode)) throw new BadScriptException();
        }

        public SymInstruction(OpCode opcode, ReadOnlyMemory<byte> operand)
        {
            OpCode = opcode;
            if (!Enum.IsDefined(typeof(OpCode), opcode)) throw new BadScriptException();
            Operand = operand;
        ...
        public SymInstruction(ReadOnlyMemory<byte> script, int ip) : this((OpCode)script.Span[ip++])
        {
        ...
        public static implicit operator SymInstruction(Instruction instruction)
        {
            return new SymInstruction(instruction.OpCode, instruction.Operand);
        }
    }
}
